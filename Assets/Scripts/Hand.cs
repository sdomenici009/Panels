...
public class Hand : MonoBehaviour {
	...
	void Update () {
		for(int i =0; i < cards.Count; i++)
		{
			...
			if(Vector3.Distance(cards[i].transform.position, new Vector3(i-cards.Count/2, -1.5f, -2)) > 0.1f 
				|| Vector3.Distance(cards[i].transform.localScale, new Vector3(2, 3, 0.1f)) > 0.1f)
			{
				cards[i].GetComponent<Card>().selectable = false;
				cards[i].transform.position = Vector3.MoveTowards(cards[i].transform.position, new Vector3(i-cards.Count/2, -1.5f, -2), speed * Time.deltaTime);
